<template>
    <span><button class="button-3 presente" :class="selected==1 ? '': 'unselected'" @click="toggle(1)">{{texte1}}</button></span>
    <span><button class="button-3 presente" :class="selected==2 ? '': 'unselected'" @click="toggle(2)">{{texte2}}</button></span>
</template>
...
<script setup>
  ...
  const props= defineProps(['val','texte1','texte2','val1','val2'])
